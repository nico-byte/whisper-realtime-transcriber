...
class LiveAudioTranscriber(Inference, InputStreamGenerator):
    ...
    async def __ainit__(self, model_type: str="pretrained", model_size: str="small", device: str=None, 
                        samplerate: int=16000, blocksize: int=8000, silence_ratio: int=3500, adjustment_time: int=5, silence_threshold: int=1500/15):
    ...
    async def transcribe(self, loop_forever: bool=True, execution_interval: int=3) -> Tuple[List[str], List[str], List[str]]:
        ...
        async for indata, _ in self.generate():
            ...
            if self.global_ndarray is not None and self.silence_counter >= (self.SAMPLERATE / self.BLOCKSIZE) * execution_interval:
                ...
                if loop_forever:
                    ...
                    return self.transcript, self.original_tokens, self.processed_tokens
